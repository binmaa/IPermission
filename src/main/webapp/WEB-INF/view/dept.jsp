    ...
    $(function() {
            ...
            $.ajax({
                ...
                success:function (result) {
                    if(result.ret){
                        ...
                        $("#deptList").html(rendered);//添加首层
                        recursiveRenderDept(result.data);//循环添加其他层
                        bindDeptClick();
                    }else{
        ...
        function renderUserListAndPage(result,url){
            if(result.ret){
                ...
                var pageSize = $("#pageSize").val();
                var pageNo = $("#userPage .pageNo").val() || 1;
                renderPage(url,result.data.total,pageSize,result.data.total > 0 ? result.data.length : 0 ,"userPage",renderUserListAndPage);//?
            }else{
        ...
        function recursiveRenderDeptSelect(deptList,level){
            level = level|0;
            if(deptList && deptList.length > 0){
                deptList.forEach(function (dept,i) {
                    deptMap[i]=dept;
                    var blank="";
                    if(level > 1){
